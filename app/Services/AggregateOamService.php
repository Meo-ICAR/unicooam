        ...
        */
            ...
            foreach ($aggregates as $row) {
                OamSemestrale::create([
                    ...
                    'erogato_lordo' => $row->erogato_lordo ?? 0.00,
                    'erogato_lavorazione' => $row->erogato_lavorazione ?? 0.00,
                    'provv_clientela' => $row->provv_clientela ?? 0.00,
                    'provv_istituto_comp' => $row->provv_istituto_comp ?? 0.00,
                    'premi_istituto_comp' => $row->premi_istituto_comp ?? 0.00,
                    'payin_ass_banche' => $row->payin_ass_banche ?? 0.00,
                    'payin_ass_broker' => $row->payin_ass_broker ?? 0.00,
                    'payin_ass_broker_cap' => $row->payin_ass_broker_cap ?? 0.00,
                    'payout_rete_credito' => $row->payout_rete_credito ?? 0.00,
                    'payout_rete_ass_banche' => $row->payout_rete_ass_banche ?? 0.00,
                    'payout_rete_ass_broker' => $row->payout_rete_ass_broker ?? 0.00,
                    'payout_rete_ass_broker_cap' => $row->payout_rete_ass_broker_cap ?? 0.00,
                    'importo_retrocesse' => $row->importo_retrocesse ?? 0.00,
                ]);
            }
